...
Get-NetFirewallRule -DisplayName "AEGIS PC Server $Port" -ErrorAction SilentlyContinue |
    Remove-NetFirewallRule -ErrorAction SilentlyContinue
if ($Bind -notin @("127.0.0.1", "::1", "localhost")) {
    if (-not $AllowedRemoteAddress) {
        throw "-AllowedRemoteAddress is required for a non-loopback bind"
    }
    New-NetFirewallRule `
        -DisplayName "AEGIS PC Server $Port" `
        -Direction Inbound `
        -Action Allow `
        -Protocol TCP `
        -LocalPort $Port `
