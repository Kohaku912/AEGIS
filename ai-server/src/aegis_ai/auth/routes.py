...
async function login(){ $('status').textContent='パスキーを確認しています...'; const o=await post('/auth/passkey/login/options'); const cred=await navigator.credentials.get({publicKey:publicKeyGetOptions(o)}); await post('/auth/passkey/login/verify',{challenge_id:o.challenge_id,credential:authCredential(cred)}); location.href='/dashboard';}
