...
async function login(){ $('status').textContent='パスキーを確認しています...'; const o=await post('/auth/passkey/login/options'); const cred=await navigator.credentials.get({publicKey:publicKeyGetOptions(o),mediation:'optional'}); await post('/auth/passkey/login/verify',{challenge_id:o.challenge_id,credential:authCredential(cred)}); location.href='/dashboard';}
...
async function init(){const me=await fetch('/auth/me').then(r=>r.json()); $('setup').hidden=!!me.bootstrap?.has_users; if(window.PublicKeyCredential && PublicKeyCredential.isConditionalMediationAvailable){try{if(await PublicKeyCredential.isConditionalMediationAvailable()) login();}catch(e){}}}
