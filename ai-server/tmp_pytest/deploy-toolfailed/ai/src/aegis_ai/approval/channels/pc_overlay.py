    ...
    def _handle(self, event: ApprovalEvent) -> bool:
        ...
        try:
            ...
            if not self._successful_result(result, require_decision=True):
                # Soft-fail: PC offline or overlay unavailable — Dashboard/Android remain.
                err = ""
                if isinstance(result, dict):
                    err = str(result.get("error") or result.get("message") or "")
                logger.info(
                    "PC overlay not delivered for %s (%s); other channels remain available",
                    event.approval_id,
                    err[:160] or "unreachable",
                )
                return False
            ...
            logger.info(
                "PC overlay delivery skipped for %s: %s",
                event.approval_id,
                str(exc)[:200],
            )
            return False
