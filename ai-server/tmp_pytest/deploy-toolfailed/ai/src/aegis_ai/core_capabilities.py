...
class AegisCoreCapabilityClient:
    ...
    def _search_web(self, params: dict[str, Any]) -> dict[str, Any]:
        ...
        if not query:
            return {"ok": False, "error": "query is required", "code": "INVALID_ARGUMENT"}
        try:
            max_results = min(max(int(params.get("max_results") or 5), 1), 20)
        except (TypeError, ValueError):
            max_results = 5
        from aegis_ai.integrations.duckduckgo_search import DuckDuckGoSearch

        response = DuckDuckGoSearch().search(query, max_results=max_results)
        if not response.success:
            return {
                "ok": False,
                "error": response.error or "web search failed",
                "code": "SEARCH_FAILED",
                "query": query,
            }
        results = [
        ...
        return {
            ...
            "results": results,
            "count": len(results),
            "result": response.to_context_string(2000),
        }
