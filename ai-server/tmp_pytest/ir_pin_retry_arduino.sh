#!/bin/bash
# Full header pin IR sweep with Arduino-like LSB-first variants + USB cam check.
set -euo pipefail
SSHPASS=$(cat /tmp/opi_pw.txt); export SSHPASS
HOST=192.168.50.120
OUT=/tmp/aegis-cam/pin_sweep3
mkdir -p "$OUT"
...
for entry in "${PINS[@]}"; do
  ...
  sshpass -e ssh -o StrictHostKeyChecking=no -o UserKnownHostsFile=/dev/null \
    ...
    "PYTHONPATH=/opt/aegis/room-server/src /opt/aegis/room-server/.venv/bin/python /tmp/opi_pin_blast.py $soc"
  sleep 1.5
...
# Early stop if clear darkening (light off)
if row["delta"] <= -20:
