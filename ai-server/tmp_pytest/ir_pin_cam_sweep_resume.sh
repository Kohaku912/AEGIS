#!/bin/bash
# Resume pin sweep from board pin 19 onward.
set -euo pipefail
...
for entry in "${PINS[@]}"; do
  ...
  if grep -q "\"soc\": \"$soc\"" "$results" 2>/dev/null; then
    echo "skip $soc already done"
    continue
  fi
...
for line in (out / "results.jsonl").read_text().splitlines():
    r = json.loads(line)
    jpg = out / f"{r['label']}.jpg"
    b = float(r.get("brightness") or (bright(jpg) if jpg.exists() else -1))
    rows.append({
        "board": r.get("board"),
        "soc": r.get("soc"),
        "brightness": b,
...
print(json.dumps({"baseline": base_b, "count": len(rows), "ranked_by_delta": rows}, ensure_ascii=False, indent=2))
best = rows[0] if rows else None
if best and best["delta"] <= -15:
    print("LIKELY_HIT", best)
else:
    print("NO_CLEAR_HIT", best)
PY
