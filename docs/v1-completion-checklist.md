...
- [ ] `.env`, data volume, auth DB, capability overrides, audit, memory, and reports persist across restart/rebuild.
...
- [ ] ToolBroker/TaskExecutionEngine uses VerificationService status for step terminal state.
- [ ] FAILED flows into retry/repair; REQUIRES_OBSERVATION flows into observation/user confirmation.
- [ ] PC verification uses screenshot/active_window/app_state before/after signals.
- [ ] Android verification uses ui_tree/current_app/permission before/after signals.
- [ ] Browser verification uses DOM selector/text/http status, not URL only.
- [ ] Verification results and suggested recovery are visible in Audit/Event/Task detail.
- [x] Capability coverage audit is included in Readiness Gate.

## 6. P1 LLM, Memory, Autonomy

- [ ] All LLM calls record input/output/context/cache/cost audit detail.
- [ ] ContextBuilder always passes system/history/memory/events/capability/tool_schema/user_state breakdown.
- [ ] LLM Usage shows context breakdown, retry loop, prompt usage, and heavy memory cases.
- [ ] Retry loop detection uses dedup-before raw audit entries.
- [ ] All prompts are classified as used/unused/dead_runtime/test_only/duplicated/hardcoded_candidate.
- [ ] Memory interface is organized as episodic/semantic/procedural.
- [ ] ContextBuilder exposes memory_budget_tokens, memory_top_k, and memory_reason.
- [ ] AutonomousLoop suppresses autonomous LLM use when usage is high.
- [ ] AutonomousLoop uses user_commitment/system_health/learning/curiosity-level decision axes.
- [ ] Autonomous execution never treats mock success as real success.

## 7. P1 Security

- [ ] Production mode rejects unauthenticated Dashboard/API/SSE/WebSocket access.
- [ ] Fresh passkey auth is required for risk changes, approvals, secrets/LLM config, and dangerous actions.
- [ ] CSRF is enforced for POST/PUT/DELETE APIs.
- [ ] Display token is GET-only and local/token limited.
- [ ] Dashboard access token is bootstrap/recovery only.
- [ ] Secret scanning confirms no secrets in image, Git, or static assets.
- [ ] Backup and restore have been executed once.
- [ ] gRPC TLS status is documented as integrated or intentionally not used for v1.
- [ ] WAN exposure is prohibited except Cloudflare/Tailscale-controlled entrypoints.

## 8. P2 Packaging And Operations

- [ ] PC Server portable zip can be built.
- [ ] PC Server service install/uninstall is verified on Windows.
- [ ] PC Server logs/config/firewall paths are documented.
- [ ] NSIS installer plan is documented.
- [ ] Ubuntu install/start/healthcheck/systemd are verified on real host.
- [ ] AI dedicated Display opens only Presentation/Display surfaces.
- [ ] Display backlight turns off when idle and wakes on relevant events.
- [ ] Device IP/MAC/role inventory exists locally on each device.
- [ ] `.aegis-local/` is gitignored and checked for accidental commits.
- [ ] Logs/reports/audit retention and rotation are defined.

## 9. P2 Documentation

- [ ] Roadmap reflects current implementation.
- [ ] Implementation status is generated or audit-backed.
- [ ] Risk register reflects TLS, Docker, notifications, external integrations, and Room status.
- [ ] Backlog is production-readiness centered.
- [ ] Testing docs include real-device markers and skip conditions.
- [ ] Ubuntu runbook covers Cloudflare, Tailscale, backup/restore, and systemd recovery.
- [ ] PC Server production docs cover service, installer, and real-action E2E.
- [ ] Android docs cover LAN-outside/Tailscale/reconnect/permission runbooks.
- [ ] Room docs cover Orange Pi migration and mock prohibition.
- [ ] UI docs, summary, checklist, and audit report are consistent.

## 10. v1 Completion Gate

- [ ] `cd ai-server && pytest` passes.
- [ ] `cd web-ui && npm test -- --run` passes.
- [ ] `cd web-ui && npm run build` passes.
- [ ] `cd web-ui && npx playwright test` passes for major UI states.
- [ ] `cd android-server && ./gradlew.bat :app:assembleDebug` passes.
- [ ] Android observe/approval/reconnect real E2E passes.
- [ ] PC service observe/approval/action real E2E passes.
- [ ] Browser/Dev real checks pass.
- [ ] Docker restart/rebuild persistence passes.
- [ ] Cloudflare login and Dashboard auth redirect pass.
- [ ] Dedicated Display kiosk recovers after reboot.
- [ ] `scripts/e2e/run-readiness-report.ps1` reports `overall_status: pass`.
- [ ] Production blocker mock/stub count is zero, or all remaining items are v1-scope excluded.
- [ ] 72-hour-equivalent Display/server soak shows no memory/SSE/UI degradation.
