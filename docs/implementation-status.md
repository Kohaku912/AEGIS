...
As of 2026-07-14 this document is descriptive; release truth is generated by
`scripts/audit-production-readiness.py`, `scripts/audit-v1-completion.py`, and
the E2E summary. Docker private binds, persistence, log rotation, passkey auth,
LLM usage measurement, completion verification, and UI Overview v3 are
implemented. Ubuntu reboot recovery, PC real actions, Android reconnect, and
the full soak remain acceptance checks until their current-host reports pass.
Room is intentionally `UNCONFIGURED/DISABLED` pending its Orange Pi real
provider. gRPC TLS remains partial and v1 relies on Tailscale/private transport.
